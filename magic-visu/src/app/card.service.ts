...
export class CardService {
  ...
  getCards(): Observable<Card[]> {
    ...
    if (!this._cards) { 
      this._cards = this.http.get<Card[]>('/assets/db_similarities_small.json').pipe(publishReplay()).refCount();
    } 
    return this._cards;
  }
  ...
  getCard(id: number): Observable<Card> {
    ...
    return this.getCards().pipe(map(cards => cards.filter(card => card.multiverseid === id)[0]));
  }
}
